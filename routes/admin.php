...
Route::middleware(['auth', 'auth.type:admin,super-admin'])->prefix('/admin')->group(function () {
    ...
    Route::get('/users/{user}/profile', [UsersController::class, 'profile'])->name('profile.view');

    Route::get('users/{user}/qrcode', function () {

        $item = Profile::all();

        return QrCode::size(300)->generate(route('profile.view', $item->user_id));
        })->name('qrcode.users');
